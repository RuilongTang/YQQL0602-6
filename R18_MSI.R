...
###Expression levels of high and low-risk groups ---------------------------
exp <- msi1[,c(2,3)]
exp$group <- ifelse(exp$MSI == "NO","MSI-L/MSS","MSI-H")

p<-ggboxplot(exp, x = "group", y = "riskScore",
             color = "group", palette = c("#F2F921", "#1E90FF"),
             add = "jitter",
             short.panel.labs = T,
             ggtheme = theme_bw())+
  ...
  ggtitle("") +
  geom_signif(comparisons = list(c("MSI-L/MSS","MSI-H")),
              test = wilcox.test,
              map_signif_level = T,
              y_position = 1.5)+
  ...
  guides(fill='none')
p
ggsave(filename = '02.msi_risk.pdf',p,w=7,h=6)
ggsave(filename = '02.msi_risk.png',p,w=7,h=6)


###Analysis of Survival Differences-------------------------------
###Calculate the survival value
msi2 <- msi1
msi2$group <- ifelse(msi2$MSI == "NO","MSI-L/MSS","MSI-H")
write.csv(msi2,file = "MSI.csv",quote=F,row.names=F)


kmfit <- survfit(Surv(OS.time,OS) ~ group, data = msi2)
print(kmfit)
##draw
pdf(file = '03.KM.pdf',w=8,h=7,onefile = F)
ggsurvplot(kmfit,
           pval = TRUE,
           conf.int = F,
...
png(file = '03.KM.png',w=700,h=600)
ggsurvplot(kmfit,
           pval = TRUE,
           conf.int = F,
           legend.labs=c("MSI-H","MSI-L/MSS"),
